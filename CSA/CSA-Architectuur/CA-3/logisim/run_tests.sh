...
circ_file=$(find . ALU/ -type f -name "ALU_Group*.circ" | head -n 1)

# Check if any .circ file was found; if not, exit with an error message
if [[ -z $circ_file ]]; then
  echo "Error: No ALU_Group*.circ file found in the current or ALU directory."
  exit 1
fi

# Run tests if the --donttest flag is not set
if [[ $1 != --donttest ]]; then
	for test_file in $(find ALU/ -type f -name "*test.txt"); do
		python3.11 Test_2425_zit1.py -c "$circ_file" -a -i "$test_file"
	done
fi
