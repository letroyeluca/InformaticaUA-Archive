...
namespace img {
    ...
    public:
        ...
        void doProjectionOnPoint(const Vector3D &point, double d);
    ...
    public:
        ...
        bool isValid(const int x, const int y, const double zvalue);
    };
    ...
    public:
        ...
        void draw_line(const Line2D &);

        void draw_zbuf_line(const Line2D &line, ZBuffer &);

        void draw_zbuf_triangle(ZBuffer &zbuf, const Vector3D &A,
                                const Vector3D &B, const Vector3D &C,
                                double d, double dx, double dy,
                                const img::Color &color);

        void draw_zbuf_line(ZBuffer &, unsigned int x0, unsigned int y0,
                            double z0, unsigned int x1, unsigned int y1,
                            double z1, const img::Color &color);

        static EasyImage draw2DLinesNoZBuf(std::vector<Line2D> &lines,
                                           int size,
                                           const img::Color &backgroundcolor);

        static EasyImage
        draw2DLinesZBuf(std::vector<Line2D> &lines, int size,
...
struct ZBuffer : public std::vector<std::vector<double> > {
    // Constructor: creates a Z-Buffer of the correct size
    // and initializes all fields to -inf
