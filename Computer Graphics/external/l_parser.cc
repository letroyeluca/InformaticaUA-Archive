...
namespace {
    ...
    public:
        stream_parser(std::istream &stream) : in(stream), ex(stream, std::istream::failbit | std::istream::badbit),
                                              line(1), col(0), endOfLineRead(false) {
            in.clear();
        }
    ...
    void print_system(std::ostream &out, LParser::LSystem const &system) {
        if (system.get_alphabet().size() == 0) {
            ...
            out << "Rules = {" << *system.get_alphabet().begin() << " -> \"" << system.get_replacement(
                *system.get_alphabet().begin()) << "\"}" << std::endl;
        } else {
            ...
            out << "Draw = {" << *system.get_alphabet().begin() << "-> " << (system.draw(*system.get_alphabet().begin())
                                                                                 ? 1
                                                                                 : 0) << "";
            for (std::set<char>::const_iterator i = ++system.get_alphabet().begin(); i != system.get_alphabet().end(); i
                 ++) {
        ...
        while (true) {
            ...
            if (value != 0 && value != 1)
                throw LParser::ParserException(
                    std::string("Invalid draw specification for entry '") + alphabet_char + "' in draw specification",
                    parser.getLine(), parser.getCol());
    ...
    void parse_rules(std::set<char> const &alphabet,
                     std::map<char, std::vector<std::pair<double, std::string> > > &rules, stream_parser &parser,
                     bool parse2D) {
        parser.skip_comments_and_whitespace();
        ...
        while (true) {
            ...
            if (alphabet.find(c) == alphabet.end())
                throw LParser::ParserException(
                    std::string("Replacement rule specified for char '") + c + "' which is not part of the alphabet. ",
                    parser.getLine(), parser.getCol());
            ...
            // Check if the next character is '{' for the new format
            if (parser.peekChar() == '{') {
                parser.assertChars("{");
                parser.skip_comments_and_whitespace();

                // Parse the rules with probabilities
                while (true) {
                    ...
                    // Add the rule with its probability
                    rules[alphabet_char].push_back({probability, rule});
            ...
            } else // Old format without probabilities
            {
                std::string rule = parser.readQuotedString();
                if (!isValidRule(alphabet, rule, parse2D))
                    throw LParser::ParserException(
                ...
                // Add the rule with default probability of 1.0
                rules[alphabet_char].push_back({1.0, rule});
            }
...
LParser::ParserException::ParserException(std::string const &msg, unsigned int line,
...
LParser::LSystem::LSystem() : alphabet(), drawfunction(), initiator(""), angle(0.0), replacementrules(),
                              nrIterations(0) {
}

LParser::LSystem::LSystem(LSystem const &system) : alphabet(system.alphabet), drawfunction(system.drawfunction),
                                                   initiator(system.initiator), angle(system.angle),
                                                   replacementrules(system.replacementrules),
...
std::string LParser::LSystem::get_replacement(char c) const {
    assert(get_alphabet().find(c) != get_alphabet().end());
    const auto &rules = replacementrules.find(c)->second;
    if (rules.size() == 1) {
        return rules[0].second;
    }

    // Store weights and strings separately for easier selection
    std::vector weighted_rules(rules.begin(), rules.end());

    // Compute total weight
    double total_weight = 0.0;
    for (const auto &rule: weighted_rules)
        total_weight += rule.first; // Sum of weights

    assert(total_weight > 0.0); // Ensure at least one valid rule

    // Generate a random number in range [0, total_weight)
    ...
    // Pick the corresponding rule
    double cumulative_weight = 0.0;
    for (const auto &rule: weighted_rules) {
        cumulative_weight += rule.first;
        if (random_value <= cumulative_weight)
            return rule.second; // Return the chosen string
    }

    return weighted_rules.back().second; // Fallback (shouldn't reach here)
}
...
LParser::LSystem2D::LSystem2D() : LSystem(), startingAngle(0.0) {
}

LParser::LSystem2D::LSystem2D(LSystem2D const &system) : LSystem(system), startingAngle(system.startingAngle) {
}

LParser::LSystem2D::LSystem2D(std::istream &in) : LSystem(), startingAngle(0.0) {
    in >> *this;
}
...
LParser::LSystem3D::LSystem3D() : LSystem() {
}

LParser::LSystem3D::LSystem3D(LParser::LSystem3D const &system) : LSystem(system) {
}

LParser::LSystem3D::LSystem3D(std::istream &in) : LSystem() {
    in >> *this;
}
