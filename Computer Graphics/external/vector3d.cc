...
Vector3D &Vector3D::operator-=(const Vector3D &rhs) {
    ...
    infty = infty == rhs.infty;
    // if we're subtracting two points or two vectors the result is a vector, otherwise it's a point
    return *this;
}
...
Vector3D Vector3D::rotate(const Vector3D &v, double angle,
                          const Vector3D &axis) {
    Vector3D k = axis;
    k.normalise();
    ...
    return v * cosTheta + cross(k, v) * sinTheta + k * (k.dot(v) * (1 - cosTheta));
}
