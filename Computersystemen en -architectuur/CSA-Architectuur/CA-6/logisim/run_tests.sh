...
circ_file=$(find . -type f -name "FD_Group*.circ" | head -n 1)

# Check if any .circ file was found; if not, exit with an error message
if [[ -z $circ_file ]]; then
  echo "Error: No FD_Group*.circ file found in the current directory"
  exit 1
fi

# Run tests if the --donttest flag is not set
if [[ $1 != --donttest ]]; then
	for test_file in $(find ./FD/ -type f -name "*test.txt"); do
