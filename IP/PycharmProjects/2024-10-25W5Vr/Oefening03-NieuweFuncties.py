...
def print_sudoku(s):
    ...
    for x in range(9):
        ...
        if ((x+1)%3 == 0):
            ...
            print("|                 |                 |                 |")
def mogelijk(sudoku, x, y):
    """
    Check welke getallen mogelijk zijn op bepaalde posities
    """
    if sudoku[x][y] is not None:
        return []
    mogelijk = set(range(1, 10))
    for waarde in sudoku[x]:
        if waarde in mogelijk:
            mogelijk.remove(waarde)
    for rij in range(9):
        if sudoku[rij][y] in mogelijk:
            mogelijk.remove(sudoku[rij][y])
    for i in range(3):
        for j in range(3):
            waarde = sudoku[i + (3*(x // 3))][j + (3*(y // 3))]
            if waarde in mogelijk:
                mogelijk.remove(waarde)
    return list(mogelijk)
def nine_by_nine():
    """
    Return tuples for a nine by nine grid
    """
    nine_by_nine = []
    for i in range(9):
        for j in range(9):
            nine_by_nine.append((i, j))
    return nine_by_nine
def vul_aan(sudoku):
    """
    Vul een sudoku aan voor zo lang als mogelijk
    """
    changed = True
    nbn = nine_by_nine()
    while changed:
        changed = False
        for e in nbn:
            if sudoku[e[0]][e[1]] is None:
                m = mogelijk(sudoku, e[0], e[1])
                if len(m) == 1:
                    sudoku[e[0]][e[1]] = m[0]
                    changed = True
